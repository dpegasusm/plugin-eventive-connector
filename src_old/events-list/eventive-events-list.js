( function () {
	...
	function fetchEventsOnce( bucket, opts ) {
		opts = opts || {};
		const includePast = !! opts.includePast; // default false
		const includeVirtual = !! opts.includeVirtual; // default false (filter client-side as well)
		const forceAll = !! opts.forceAll; // when true, do NOT use upcoming_only so undated events are included
	...
	function initContainer( container ) {
		if ( ! container || container.__eventiveInited ) {
			return;
		}
		...
		const loaderId = container.id.replace( /_container$/, '_loader' );
		const loader = document.getElementById( loaderId );

		const bucket = container.getAttribute( 'data-event-bucket' ) || '';
		const limit =
			parseInt( container.getAttribute( 'data-limit' ) || '0', 10 ) || 0;
		const tagDefault = container.getAttribute( 'data-tag-default' ) || '';
		const tagsListRaw = container.getAttribute( 'data-tags-list' ) || '';
		...
		const excludeTokens = excludeTagsRaw
			? excludeTagsRaw
					...
					.filter( Boolean )
			: [];
		const venueId = container.getAttribute( 'data-venue-id' ) || '';
		// Short / full description visibility
		// Support both data-show-short-description and data-short-event-description
		const rawShortDescAttr =
			container.getAttribute( 'data-show-short-description' ) ||
			container.getAttribute( 'data-short-event-description' );
		const showShortDesc =
			rawShortDescAttr == null ? false : parseBool( rawShortDescAttr );

		// Support both data-show-description and data-event-description
		const rawDescAttr =
			container.getAttribute( 'data-show-description' ) ||
			container.getAttribute( 'data-event-description' );
		const showDescription =
			rawDescAttr == null ? false : parseBool( rawDescAttr );

		const showPoster =
			container.getAttribute( 'data-show-poster' ) === 'true';
		const imagePref =
			container.getAttribute( 'data-image-preference' ) || 'cover';
		const includeVirtual =
			container.getAttribute( 'data-include-virtual' ) === 'true';
		const includePast =
			container.getAttribute( 'data-include-past' ) === 'true'; // default false
		const viewMode = container.getAttribute( 'data-view-mode' ) || 'list';
		// Show undated events (defaults to true)
		let showUndated = container.getAttribute( 'data-show-undated' );
		showUndated =
			showUndated === null ||
			showUndated === '' ||
			showUndated === 'true';
		...
		const startDateStr = container.getAttribute( 'data-start-date' ) || '';
		...
		const filmDetailBaseAttr = container.getAttribute(
			'data-film-detail-base'
		);
		if ( filmDetailBaseAttr ) {
			__EventiveFilmDetailBase = filmDetailBaseAttr;
		}
		const filmDetailPrettyAttr = container.getAttribute(
		...
		const showFilter = parseBool(
			container.getAttribute( 'data-show-filter' )
		);
		...
		if ( ! showFilter ) {
			try {
				...
				if ( parent ) {
					parent
						.querySelectorAll( '.eventive-events-tags-filter' )
						.forEach( function ( el ) {
		...
		function collectTagsFromEvents( list ) {
			const map = new Map(); // key -> {id,name,color,count}
			( list || [] ).forEach( function ( ev ) {
				...
				evTags.concat( fmTags ).forEach( function ( t ) {
					...
					if ( excludeTokens && excludeTokens.length ) {
						...
						const shouldExclude = excludeTokens.some(
							function ( tok ) {
								const idTok = String( tok );
								const nameTok = String( tok ).toLowerCase();
								return (
									( id && idTok === id ) ||
									( lowerName && nameTok === lowerName )
								);
							}
						);
						if ( shouldExclude ) {
		...
		function renderEventTags( list ) {
			...
			const tags = collectTagsFromEvents( list );
			if ( ! tags.length ) {
				tagsWrap.innerHTML = '';
				return;
			}
			...
			const html =
				'<div class="tag-container eventive-tags">' +
				'<span class="tag-label is-all"><a class="external-tag-filter" data-tag-id="" href="' +
				resetUrl +
				'">All</a></span>' +
				tags
					.map( function ( t ) {
						const fg = getTextColor( t.color || '#e0e0e0' );
						const href = ( function () {
							try {
			...
			if ( ! showFilter && tagsWrap ) {
				tagsWrap.classList.add( 'is-hidden' );
					...
					function ( e ) {
						const a =
							e.target && e.target.closest
								? e.target.closest( 'a.external-tag-filter' )
								: null;
						if ( ! a ) {
							return;
						}
						...
						highlightActiveTag();
						if ( Array.isArray( window.allEventiveEvents ) ) {
							render( window.allEventiveEvents, activeTag );
						}
					},
					true
				);
		...
		function isUpcoming( ev ) {
			...
			if ( ! isFinite( t ) ) {
				return false;
			} // skip malformed dates when upcoming-only
			return t >= Date.now();
		}
		...
		function render( events, overrideTag ) {
			...
			( events || [] ).forEach( function ( event ) {
				...
				allTagObjs.forEach( function ( t ) {
					...
					if ( t.name ) {
						eventTagNames.push( String( t.name ).toLowerCase() );
					}
				} );

				if ( filterTokens.length ) {
					const matches = filterTokens.some( function ( tok ) {
						const idTok = String( tok );
						const nameTok = String( tok ).toLowerCase();
						return (
							eventTagIds.indexOf( idTok ) !== -1 ||
							eventTagNames.indexOf( nameTok ) !== -1
						);
					} );
					if ( ! matches ) {
						return;
				...
				if (
					...
					if ( onlyTag ) {
						...
						if ( onlyId || onlyName ) {
							const lowerOnlyName =
								String( onlyName ).toLowerCase();
							const onlyExcluded = excludeTokens.some(
								function ( tok ) {
									const idTok = String( tok );
									const nameTok = String( tok ).toLowerCase();
									return (
				...
				if ( isUndated ) {
					...
					if ( ! isUpcoming( event ) ) {
						return;
					} // enforce upcoming/past rule for dated events
					dated.push( event );
				}
			} );

			// 2) Order: undated first, then dated; respect limit AFTER ordering
			let ordered = undated.concat( dated );
			if ( limit && limit > 0 ) {
			...
			ordered.forEach( function ( event ) {
				...
				if ( event.films && event.films.length ) {
					filmTitlesHtml = event.films
						.map( function ( f ) {
							if ( ! f || ! f.name ) {
								return '';
							}
							const url = resolveFilmUrl( f );
							const inner = url
								? '<a href="' +
								  url +
								  '" class="events-film-link">' +
								  f.name +
								  '</a>'
								: f.name;
							return (
								'<span class="events-film-title">' +
								inner +
								'</span>'
							);
						} )
				...
				if ( event.films && event.films.length ) {
					if ( event.films.length === 1 ) {
						const singleFilm = event.films[ 0 ];
						const singleUrl = resolveFilmUrl( singleFilm );
						const singleInner = singleUrl
							? '<a href="' +
							  singleUrl +
							  '" class="events-film-link view-film-details">View Film Details</a>'
							: 'View Film Details';
						filmMetaHtml =
							'<div class="event-film-list single-film">' +
							singleInner +
							'</div>';
					} else {
						filmMetaHtml =
							'<div class="event-film-list"><em>Showing:</em> ' +
							filmTitlesHtml +
							'</div>';
					}
				}
				...
				if ( showPoster && imagePref !== 'none' && imageUrl ) {
					...
					if ( event.films && event.films.length === 1 ) {
						const posterFilm = event.films[ 0 ];
						const posterUrl = resolveFilmUrl( posterFilm );
						if ( posterUrl ) {
							posterInner =
								'<a href="' +
								posterUrl +
								'" class="event-poster-link">' +
								posterInner +
								'</a>';
						}
					}
				...
				( event.tags || [] )
					.concat(
						( event.films || [] ).flatMap( function ( f ) {
							return f.tags || [];
						} )
					)
				...
				const tagLabels = Array.from( tagMap.values() )
					.map( function ( tag ) {
						const bg = tag.color || '#888',
							fg = getTextColor( bg );
						return (
							'<span class="event-label" style="background-color:' +
							bg +
							'; color:' +
							fg +
							'">' +
				...
				if ( viewMode === 'grid' ) {
					...
					html.push(
						'<div class="event-grid-card">' +
							...
							'</h3>' +
							( tagLabels
								? '<div class="event-grid-tags">' +
								  tagLabels +
								  '</div>'
								: '' ) +
							'</div>' +
							( ticketBtn
								? '<div class="event-grid-ticket">' +
								  ticketBtn +
								  '</div>'
								: '' ) +
							'</div>'
						...
						'<div class="event-list-item entry">' +
							...
							( startHtml
								? '<p class="event-list-time">' +
								  ...
								  '</p>'
								: '' ) +
							'<h3 class="event-list-name entry-title">' +
							name +
							'</h3>' +
							filmMetaHtml +
							...
							tagLabels +
							'</div>' +
							( showShortDesc && shortDesc
								? '<p class="event-list-short-description">' +
								  shortDesc +
								  '</p>'
								: '' ) +
							( showDescription
								? '<p class="event-list-description entry-content">' +
								  desc +
								  '</p>'
								: '' ) +
							'</div>' +
							( ticketBtn
								? '<div class="event-list-buy-tickets">' +
								  ticketBtn +
								  '</div>'
								: '' ) +
							'</div>'
			...
			const run = function () {
				...
				fetchEventsOnce( bucket, {
					...
					.catch( function () {
						...
						container.innerHTML =
							'<p class="error-message">Error fetching events.</p>';
					} );
			};
			if ( Eventive._ready || Eventive.ready ) {
				run();
			} else {
	...
	function initAll() {
		document
			.querySelectorAll( '.eventive-events-list' )
