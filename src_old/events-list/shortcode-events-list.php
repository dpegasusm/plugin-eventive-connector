...
function eventive_schedule_list( $atts ) {
	...
	wp_enqueue_style(
		...
		plugin_dir_url( __DIR__ ) . 'css/events-list.css',
		array(),
		'1.0.0'
	);
	...
	wp_enqueue_style(
		'eventive-tags-style',
		plugin_dir_url( __DIR__ ) . 'css/eventive-tags.css',
		array(),
		'1.0.0'
	);
	...
	$view_mode          = in_array( strtolower( $atts['view'] ), array( 'grid', 'list' ), true ) ? strtolower( $atts['view'] ) : 'list';
	...
	wp_enqueue_script(
		...
		plugin_dir_url( __DIR__ ) . 'js/eventive-events-list.js',
