...
function eventive_schedule( $atts ) {
	...
	$image_mode       = in_array( strtolower( $atts['image'] ), array( 'poster', 'cover', 'still' ), true ) ? strtolower( $atts['image'] ) : 'poster';
			...
			if (window.Eventive && typeof Eventive.request === 'function') {
				Eventive.on('ready', function () {
					...
					const queryParams = {
						...
						<?php
						if ( $tag_id ) {
							echo "'tag-id': " . json_encode( $tag_id ) . ',';}
						?>
						<?php
						if ( $venue_id ) {
							echo "'venue-id': " . json_encode( $venue_id ) . ',';}
						?>
					};
