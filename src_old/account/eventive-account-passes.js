( function () {
	...
	function isModal( el ) {
		return !! (
			el &&
			( el.classList.contains( 'eventive-edit-pass-modal' ) ||
				el.classList.contains( 'eventive-show-pass-barcode-modal' ) )
		);
	}
	...
	function fetchPasses( bucket ) {
		...
		if ( PASS_CACHE[ key ] ) {
			return Promise.resolve( PASS_CACHE[ key ] );
		}
		if ( ! window.Eventive ) {
	...
	function renderPassCard( pass, idx ) {
		const name = pickFirst( pass.name, pass.pass_name, pass.title, 'Pass' );
		const type = pickFirst( pass.type, pass.pass && pass.pass.type );
		const status =
			pass.status || ( pass.checked_in ? 'Checked in' : 'Active' );
		const benefits =
		...
		const events =
			pass.events_remaining != null
				...
				: '';
		const sale =
			pass.gross_cents != null ? currency( pass.gross_cents / 100 ) : '';

		const metaBits = [ type, events, sale ].filter( Boolean ).join( ' • ' );

		return (
			'<div class="eventive-pass-card" data-pass-idx="' +
			idx +
			'">\n' +
			'  <div class="eventive-pass-card__body">\n' +
			'    <div class="eventive-pass-card__title">' +
			( name || 'Pass' ) +
			'</div>\n' +
			'    <div class="eventive-pass-card__meta">' +
			( metaBits || '' ) +
			'</div>\n' +
			( benefits
				? '    <div class="eventive-pass-card__benefits">' +
				  benefits +
				  '</div>\n'
				: '' ) +
			'  </div>\n' +
	...
	function openEditModal( cfg, pass, idx ) {
		const modal = document.getElementById( cfg.edit_modal );
		const form = document.getElementById( cfg.edit_form );
		const nameI = document.getElementById( cfg.edit_name );
		const idxI = document.getElementById( cfg.edit_idx );
		const fieldsWrap = document.getElementById( cfg.edit_fields );
		const closeBtn = document.getElementById( cfg.edit_close );
		...
		const supp = pass.supplementary_data || pass.supplementary_fields || {};
		try {
		...
		form.onsubmit = function ( e ) {
			...
			if ( nameI && nameI.value.trim() !== '' ) {
				payload.name = nameI.value.trim();
			}

			// Collect supplementary fields
			const inputs = form.querySelectorAll( 'input[name^="supp["]' );
			if ( inputs && inputs.length ) {
				const suppObj = {};
				inputs.forEach( function ( inp ) {
					const m = inp.name.match( /^supp\[(.+)\]$/ );
					if ( m ) {
						suppObj[ m[ 1 ] ] = inp.value;
					}
				} );
			...
			// Send update
			const passId = pass.id || ( pass.pass && pass.pass.id );
			if ( ! passId ) {
	...
	function openBarcodeModal( cfg, pass ) {
		const modal = document.getElementById( cfg.barcode_modal );
		const img = document.getElementById( cfg.barcode_img );
		const meta = document.getElementById( cfg.barcode_meta );
		const legend = document.getElementById( cfg.barcode_legend );
		const close = document.getElementById( cfg.barcode_close );

		if ( ! modal || ! img ) {
			return;
		}

		const label = pickFirst( pass.name, pass.pass_name, 'Pass' );
		const codePath = pickFirst(
			pass.qr_code_path,
			pass.barcode_path,
			pass.barcode && pass.barcode.path
		);
		...
		if ( legend ) {
			legend.textContent =
				'Present this code at entry. Tip: increase screen brightness for easier scanning.';
		}
	...
	function renderPassesInto( cfg, passes ) {
		...
		if ( ! passes || ! passes.length ) {
			list.innerHTML =
				'<h2>My Passes</h2><p>No passes found for this account.</p>';
			return;
		}
		...
		const html =
			...
			passes
				.map( function ( p, i ) {
					return renderPassCard( p, i );
				} )
				.join( '' );
		list.innerHTML = html;

		// Bind actions
		list.querySelectorAll( '.eventive-pass-card .evt-btn' ).forEach(
			function ( btn ) {
				const holder = btn.closest( '.eventive-pass-card' );
				const idx = holder
					? parseInt( holder.getAttribute( 'data-pass-idx' ), 10 )
					: -1;
				if ( idx < 0 ) {
					return;
				}
				btn.addEventListener( 'click', function () {
					...
					if ( act === 'barcode' ) {
						return openBarcodeModal( cfg, pass );
					}
					if ( act === 'edit' ) {
						return openEditModal( cfg, pass, idx );
					}
				} );
			}
		);
	...
	function initOne( cfg ) {
		...
		function boot() {
			if ( ! window.Eventive || ! Eventive.on ) {
				return;
			} // fallback timer will show message if standalone
			Eventive.on( 'ready', function () {
				...
				try {
					if ( Eventive.isLoggedIn() ) {
						hide( login );
						show( list );
						fetchPasses( cfg.bucket )
							.then( function ( passes ) {
								renderPassesInto( cfg, passes );
							} )
							.catch( function ( err ) {
								console.error( 'Failed to fetch passes:', err );
								showMessage(
									list,
									'Could not load your passes.'
								);
							} );
					} else {
						// Logged out
						if ( ! inParentAccountContainer( wrap ) ) {
							showMessage(
								list,
								'You are not logged in. Please log in to view your passes.'
							);
							hide( login );
							show( list );
						} else {
					...
					showMessage(
						list,
						'There was a problem loading your passes.'
					);
					hide( login );
					show( list );
				}
	...
	function initAll() {
		const cfgs = window.__EVT_ACCOUNT_PASSES || [];
		if ( ! cfgs.length ) {
			return;
		}
