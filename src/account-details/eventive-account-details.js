( function () {
	...
	function showMessage( loginEl, contentEl, msg ) {
		...
		loginEl.innerHTML =
			...
			( msg || 'Please log in to view your account details.' ) +
			'</div>';
	}
	...
	function initOne( cfg ) {
		...
		const loginEl = document.getElementById( cfg.login );
		...
		function clearAccountDetails() {
			...
			if ( ! inParentAccountContainer( wrap ) ) {
				showMessage(
					loginEl,
					content,
					'You are not logged in. Please use the login above to continue.'
				);
			} else if ( loginEl ) {
		...
		function renderAccountDetails( details ) {
			...
			Object.keys( fields ).forEach( function ( key ) {
				...
				if ( raw === undefined ) {
					...
					if ( key === 'phone_number' ) {
						raw =
							...
							( R.phones && firstIn( R.phones, 'number' ) );
					}
					...
					if ( key === 'email' ) {
						raw =
							R &&
							( R.email ||
								( R.emails && firstIn( R.emails, 'email' ) ) );
					}
				}
				...
				if (
					...
					const parts = [
						raw.street || raw.line1,
						raw.line2,
						raw.city,
						raw.region || raw.state,
				...
				if ( key === 'sms_tickets_enabled' ) {
					...
					html +=
						...
						'<input type="checkbox" class="sms-toggle" data-key="' +
						key +
						'" ' +
						checked +
						'>\n' +
					...
					html +=
						'<button class="edit-row-button" data-key="' +
						key +
						'">Edit</button>';
				}
				html += '</td></tr>';
			...
			wrap.querySelectorAll( '.sms-toggle' ).forEach( function ( t ) {
				t.addEventListener( 'change', function () {
					...
					const cell = row && row.querySelector( '.static-value' );
					if ( cell ) {
						cell.textContent = newVal ? 'Yes' : 'No';
					}
		...
		function bindRowEditors( original ) {
			wrap.querySelectorAll( '.edit-row-button' ).forEach(
				function ( btn ) {
					btn.addEventListener( 'click', function () {
						const key = btn.getAttribute( 'data-key' );
						const row = btn.closest( 'tr' );
						const cell = row.querySelector( '.static-value' );
						const current = cell.textContent.trim();
						if ( current === 'Yes' || current === 'No' ) {
							cell.innerHTML =
								'<select class="edit-select">\n<option value="true" ' +
								( current === 'Yes' ? 'selected' : '' ) +
								'>Yes</option>\n<option value="false" ' +
								( current === 'No' ? 'selected' : '' ) +
								'>No</option>\n</select>';
						} else {
						...
						row.querySelector( 'td:last-child' ).innerHTML =
							'<button class="submit-row-button" data-key="' +
							key +
							'">Submit</button> <button class="cancel-row-button">Cancel</button>';
						row.querySelector(
							'.submit-row-button'
		...
		async function handleRowSubmit(
			...
			try {
				const personId =
					window.eventivePersonId ||
					( window.Eventive &&
						Eventive.person &&
						Eventive.person.id ) ||
					null;
				if ( ! personId ) {
					console.warn( '[account-details] missing person id' );
					return;
				}
				const url =
					( wpApiSettings.root || '/' ) +
		...
		async function fetchAccountDetails() {
			try {
				...
				if ( window.EventiveUtils && EventiveUtils.hideLoader ) {
					try {
						EventiveUtils.hideLoader( loginEl );
					} catch ( _ ) {}
				} else if ( loginEl ) {
					loginEl.style.display = 'none';
				}
				...
				const person = resp && ( resp.person || resp ); // some installs return { person: {...} }, others return the person directly
				window.eventivePersonId = person && person.id;
				const details = normalizePerson( person || {} );
				...
				if (
					...
					showMessage(
						loginEl,
						content,
						'There was a problem loading your account. Please refresh or try again later.'
					);
				}
			}
		}
		...
		function boot() {
			...
			Eventive.on( 'ready', function () {
				...
				try {
					...
					console.error( '[account-details] ready error', e );
					clearAccountDetails();
				}
			} );
		}
	...
	function initAll() {
		const cfgs = window.__EVT_ACCOUNT_DETAILS || [];
		if ( ! cfgs.length ) {
			return;
		}
