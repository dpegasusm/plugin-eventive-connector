( function () {
	...
	function rmInitialHide( container ) {
		if ( container ) {
			container.classList.remove( 'eventive-initial-hide' );
		}
	}
	...
	function dbg( msg ) {
		try {
			if (
				window.localStorage &&
				localStorage.getItem( 'EVT_LOGIN_DEBUG' ) === '1'
			) {
				console.log( '[eventive-login]', msg );
			}
		} catch ( _ ) {}
	}
	function findCfgByTriggerId( id ) {
		const list = window.__EVT_LOGIN_INSTANCES || [];
		for ( let i = 0; i < list.length; i++ ) {
			if ( list[ i ] && list[ i ].trigger === id ) {
				return list[ i ];
			}
		}
		return null;
	...
	function loginWithEventiveTokenCompat( token ) {
		const EVT =
			typeof getEVT === 'function'
				? getEVT()
				: window.__eventiveGetEVT && window.__eventiveGetEVT();
		try {
	...
	function getEVT() {
		try {
			if ( window.Eventive ) {
				return window.Eventive;
			}
			if (
				window.parent &&
				window.parent !== window &&
				window.parent.Eventive
			) {
				return window.parent.Eventive;
			}
			if ( window.top && window.top !== window && window.top.Eventive ) {
				return window.top.Eventive;
			}
		} catch ( _ ) {}
		return null;
	}
	...
	try {
		if ( typeof window.getEVT !== 'function' ) {
			window.getEVT = getEVT;
		}
		if ( typeof window.__eventiveGetEVT !== 'function' ) {
			window.__eventiveGetEVT = getEVT;
		}
	} catch ( _ ) {}
	...
	function isEVTReady( EVT ) {
		try {
			return !! (
				EVT &&
				( EVT.request ||
					( EVT.isLoggedIn && typeof EVT.isLoggedIn === 'function' ) )
			);
		} catch ( _ ) {
			return false;
		}
	...
	function renderLoggedOutUI( cfg ) {
		...
		try {
			if (
				trig &&
				cfg &&
				typeof cfg.loginLinkText === 'string' &&
				cfg.loginLinkText.trim()
			) {
	...
	function renderLoggedInUI( cfg, person ) {
		...
		if (
			! person ||
			( ! person.first_name && ! person.name && ! person.full_name )
		) {
			try {
				const EVT = getEVT();
				if ( EVT && typeof EVT.getPersonDetails === 'function' ) {
					const details = EVT.getPersonDetails();
					if ( details ) {
						name =
							details.first_name ||
							details.name ||
	...
	async function performLogin( cfg ) {
		...
		let effectiveBucket = cfg.bucket;
		try {
			if ( ( ! effectiveBucket || effectiveBucket === '' ) && EVT ) {
				effectiveBucket =
					( EVT &&
						( EVT.event_bucket ||
							( EVT.config && EVT.config.event_bucket ) ) ) ||
					'';
			}
		} catch ( _ ) {}
		...
		if ( effectiveBucket ) {
			body.event_bucket = effectiveBucket;
		}

		if ( ! effectiveBucket ) {
			if ( errEl ) {
				errEl.textContent =
					'Missing event bucket. Please set a default in settings or on this page.';
				errEl.classList.remove( 'hidden' );
			}
			if ( loginBtn ) {
		...
		async function httpLoginViaEventiveRequest() {
			const EVT = getEVT();
			if ( ! EVT || ! EVT.request ) {
				throw new Error( 'Eventive.request unavailable' );
			}
			const json = await EVT.request( {
				method: 'POST',
				path: 'people/login',
				body,
		...
		try {
			let json;
			try {
				json = await httpLoginViaFetch();
			} catch ( fetchErr ) {
				dbg( {
					action: 'login_fetch_error',
					err: String( ( fetchErr && fetchErr.message ) || fetchErr ),
				} );
				json = await httpLoginViaEventiveRequest();
			}
			...
			try {
				const personRes = await EVT.request( {
					method: 'GET',
					path: 'people/self',
					authenticatePerson: true,
				} );
				const person = personRes && ( personRes.person || personRes );
				renderLoggedInUI( cfg, person );
			} catch ( _ ) {
				renderLoggedInUI( cfg, { name: 'Friend', id: true } );
			}
	...
	function attachHandlers( cfg ) {
		...
		const trig = $( cfg.trigger ),
			modal = $( cfg.modal ),
			cancel = $( cfg.cancelBtn ),
			loginBtn = $( cfg.loginBtn );
		const logout = $( cfg.logout ),
			errEl = $( cfg.error );
		...
		if ( modal && ! modal.__closeBtnBound ) {
			...
			modal.addEventListener( 'click', function ( ev ) {
				...
				if (
					el &&
					el.classList &&
					el.classList.contains( 'eventive-modal-close' )
				) {
					ev.preventDefault();
					closeModal( modal );
				}
		...
		if ( trig && ! trig.__bound ) {
			...
			trig.addEventListener( 'click', async function ( e ) {
				...
				if ( ! EVT || ! EVT.request ) {
					dbg(
						'Eventive not ready; continuing to open modal for credentials input'
					);
				}
				...
				openModal( modal );
				bindEscToClose( modal, function () {
					closeModal( modal );
				} );
				if ( ! getEVT() ) {
					if ( errEl ) {
						errEl.textContent =
							'Eventive is still loading in the editor. Try previewing the page or viewing on the front end.';
						errEl.classList.remove( 'hidden' );
					}
				}
			} );
		...
		if ( logout && ! logout.__bound ) {
			...
			logout.addEventListener( 'click', function ( e ) {
				...
				function logoutViaRequest() {
					if ( ! EVT || ! EVT.request ) {
						return Promise.reject(
							new Error( 'Eventive.request unavailable' )
						);
					}
					return EVT.request( {
						method: 'POST',
					...
					if ( EVT && typeof EVT.logout === 'function' ) {
						...
						if ( p && typeof p.then === 'function' ) {
							p.then( function () {
								...
								clearTokens();
								logoutViaRequest().finally( function () {
									renderLoggedOutUI( cfg );
								} );
							} );
							return;
	...
	function initOne( cfg ) {
		const container = $( cfg.container );
		const EVT = getEVT();
		if ( ! container || container.__inited ) {
			return;
		}
			...
			try {
				if ( EVT.isLoggedIn && EVT.isLoggedIn() ) {
					EVT.request( {
						...
						.then( function ( res ) {
							onState( res && ( res.person || res ) );
						} )
						.catch( function () {
							onState( { id: true, name: 'Friend' } );
						} );
				} else {
					onState( null );
				}
		...
		EVT.on( 'ready', function () {
			...
			try {
				const preAuthed = !! ( EVT.isLoggedIn && EVT.isLoggedIn() );
				if ( ! preAuthed ) {
					const tok = getStoredEventiveToken();
					if ( tok ) {
			...
			try {
				let last =
					typeof cfg.__lastAuth === 'boolean' ? cfg.__lastAuth : null;
				function handleAuth( state ) {
					const person = state && ( state.person || state );
					const isAuthed = !! ( person && person.id );
					onState( person );
					if ( last === null ) {
					...
					if ( last !== isAuthed ) {
						...
						if ( isAuthed ) {
							...
							try {
								requestStorageAccessIfNeeded().then(
									function () {
										setTimeout( function () {
											try {
												window.location.reload();
	...
	if ( ! window.__EVT_LOGOUT_DELEGATE_BOUND ) {
		...
		document.addEventListener(
			...
			function ( ev ) {
				let el = ev.target;
				if ( el && el.closest ) {
					el = el.closest( 'a[id^="eventive-logout-link-"]' );
				}
				if ( ! el ) {
					return;
				}
					...
					if ( EVT && typeof EVT.logout === 'function' ) {
						...
						if ( p && typeof p.then === 'function' ) {
							p.then( function () {
								...
								clearTokens();
								logoutViaRequest().finally( hardReload );
							} );
							return;
						}
					}
	...
	if ( ! window.__EVT_LOGIN_DELEGATED_BOUND ) {
		...
		document.addEventListener(
			...
			function ( e ) {
				...
				const clickable = t.closest && t.closest( 'a[id],button[id]' );
				if ( ! clickable ) {
					return;
				}
				...
				const cfg = findCfgByTriggerId( id );
				if ( ! cfg ) {
					return;
				}
				e.preventDefault();
				...
				if ( modal ) {
					...
					dbg( 'delegated open modal for ' + id );
				}
			},
			true
		);
...
( function () {
	...
	function init() {
		...
		EVT.on( 'ready', function () {
			try {
				const authed = !! ( EVT.isLoggedIn && EVT.isLoggedIn() );
				const list = window.__EVT_LOGIN_INSTANCES || [];
					...
					try {
						if ( authed ) {
							EVT.request( {
								...
								.then( function ( res ) {
									const p = res && ( res.person || res );
									const el = document.getElementById(
										cfg.container
									);
									if ( el ) {
										el.classList.remove(
											'eventive-initial-hide'
										);
									}
									renderLoggedInUI( cfg, p );
								} )
								.catch( function () {
									const el = document.getElementById(
										cfg.container
							...
							if ( el ) {
								el.classList.remove( 'eventive-initial-hide' );
							}
							renderLoggedOutUI( cfg );
						}
					} catch ( _ ) {}
		...
		[ 'login', 'logout', 'person', 'personState', 'person_state' ].forEach(
			function ( name ) {
				try {
					EVT.on( name, function () {
						const list = window.__EVT_LOGIN_INSTANCES || [];
						list.forEach( function ( cfg ) {
							try {
								const el = document.getElementById(
