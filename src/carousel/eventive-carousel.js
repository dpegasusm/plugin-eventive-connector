 ...
 */
( function () {
	const __DBG = false; // set true for verbose console logs during troubleshooting
	const PREFERS_REDUCED = ( function () {
		try {
			return (
				window.matchMedia &&
	...
	function getCoverImage( ev ) {
		...
		const img =
			( ev.images && ( ev.images.cover_image || ev.images.cover ) ) ||
			ev.cover_image ||
			( ev.films &&
				ev.films[ 0 ] &&
				( ev.films[ 0 ].still_image ||
					ev.films[ 0 ].cover_image ||
					ev.films[ 0 ].poster_image ) );
		return img || 'default-placeholder.jpg';
	}

	function renderCarousel( container, events ) {
		if ( __DBG ) {
			console.log( '[carousel] render', events && events.length );
		}
		// Avoid double-init
		if ( container.__evtCarouselInited ) {
			return;
		}
		container.__evtCarouselInited = true;

		const limit = clamp(
			container.getAttribute( 'data-limit' ) || 10,
			1,
			50
		);
		...
		if ( ! items.length ) {
			container.innerHTML =
				'<p class="error-message">No upcoming events found.</p>';
			return;
		}
		...
		items.forEach( function ( event ) {
			...
			const showBtn = ! event.hide_tickets_button;
			...
			html.push(
				...
				'      <h3 class="carousel-title">' +
					escapeHtml( name ) +
					'</h3>',
				'      <p class="carousel-time">' +
					escapeHtml( date + ' • ' + time + venue ) +
					'</p>',
				'      <div class="carousel-ticket-button">' +
					btnHtml +
					'</div>',
				'    </div>',
				'  </div>'
		...
		setTimeout( function () {
			try {
				if (
					...
					Eventive.rebuild( container );
				} else if (
					window.Eventive &&
					Eventive.widgets &&
					typeof Eventive.widgets.build === 'function'
				) {
					Eventive.widgets.build( container );
				}
			} catch ( e ) {
				console.warn( 'Eventive rebuild failed:', e );
			}
		}, 200 );
	}
	...
	function initializeCarousel( container ) {
		const slider = bySel( container, '.carousel-slider' );
		if ( ! slider ) {
			console.error( 'Carousel slider not found (scoped).' );
			return;
		}
		const slides = bySelAll( slider, '.carousel-slide' );
		if ( ! slides.length ) {
			console.error( 'No slides.' );
			return;
		}

		const prev = bySel( slider, '.carousel-arrow.left' );
		const next = bySel( slider, '.carousel-arrow.right' );
		const dotsWrap =
			bySel( slider, '.carousel-dots' ) ||
			( function () {
		...
		const autoplay = makePausableInterval( nextSlide, 5000 );
		if ( ! PREFERS_REDUCED ) {
			autoplay.start();
		}
	...
	function fetchAndRender( container ) {
		return new Promise( function ( resolve ) {
			...
			if ( ! bucket ) {
				container.innerHTML =
					'<p class="error-message">Missing Eventive bucket.</p>';
			...
			onEventiveReady( function () {
				try {
					...
					container.innerHTML =
						'<p class="error-message">Eventive API unavailable.</p>';
					resolve( false );
				}
			} );
		} );
	...
	function initOne( container ) {
		if (
			! container ||
			container.__evtCarouselInited ||
			container.__evtObserved
		) {
			return;
		}
	...
	function boot( scope ) {
		if ( __DBG ) {
			console.log( '[carousel] boot scope', scope );
		}
		const root = scope && scope.nodeType ? scope : document;
		bySelAll( root, '.event-carousel-container[data-bucket]' ).forEach(
			initOne
		);
	}
