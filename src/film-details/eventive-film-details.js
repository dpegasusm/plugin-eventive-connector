( function () {
	...
	function render( container, film, opts ) {
		...
		container.innerHTML = `<div class="hero-section" ${
			film.cover_image
      ...
      <div class="film-details">
        ...
        <div class="film-info">
          <div><strong>Director:</strong> ${ htmlEscape(
				( film.credits && film.credits.director ) || 'Unknown'
			) }</div>
          <div><strong>Runtime:</strong> ${ htmlEscape(
				String( ( film.details && film.details.runtime ) || 'N/A' )
			) } minutes</div>
          <div><strong>Year:</strong> ${ htmlEscape(
				String( ( film.details && film.details.year ) || 'N/A' )
			) }</div>
          <div><strong>Language:</strong> ${ htmlEscape(
				String( ( film.details && film.details.language ) || 'N/A' )
			) }</div>
        </div>`
				: ''
		}
	...
	function setupTrailer( container ) {
        ...
        <div class="eventive-trailer-dialog" style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#000;border-radius:8px;display:block;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.5);width:min(90vw,960px);">
          ...
          <div class="eventive-trailer-embed" style="position:relative;padding-top:56.25%;width:100%;">
			...
			bySel( modal, '.eventive-trailer-close' ).addEventListener(
				'click',
				close
			);
			bySel( modal, '.eventive-trailer-backdrop' ).addEventListener(
				'click',
				close
			);
			modal.addEventListener( 'click', ( e ) => {
				const dialog = bySel( modal, '.eventive-trailer-dialog' );
		...
		btn.addEventListener( 'click', ( ev ) => {
			...
			if ( ! embed ) {
				console.warn(
					'[eventive-film-details] Unsupported trailer URL'
				);
				return;
			}
			iframe.src = embed;
	...
	function fetchEventsAndRenderButtons( container, cfg, opts ) {
		...
		const url = `event_buckets/${ encodeURIComponent(
			cfg.eventBucket
		) }/films/${ encodeURIComponent( cfg.filmId ) }/events`;
		window.Eventive.request( {
			...
			.then( ( resp ) => {
				let events = [];
				if ( Array.isArray( resp && resp.events ) ) {
					events = resp.events;
	...
	function initOne( cfg ) {
		...
		if ( typeof cfg.excludeVirtual === 'boolean' ) {
			...
			cfg.excludeVirtual === 'true' ||
			cfg.excludeVirtual === '1'
		) {
			// Handle string-y truthy values
			excludeVirtual = true;
		} else if (
			cfg.excludeVirtual === 'false' ||
			cfg.excludeVirtual === '0'
		) {
		...
		function start() {
			...
			window.Eventive.request( {
				...
				.then( ( film ) => {
					film = film || {};
					render( container, film, opts );
					setupTrailer( container );
					if ( opts.showEvents ) {
						fetchEventsAndRenderButtons( container, cfg, opts );
					}
				} )
				.catch( ( err ) => {
					console.error(
		...
		if ( window.Eventive && typeof window.Eventive.on === 'function' ) {
			if ( window.Eventive._ready ) {
				...
				window.Eventive.on( 'ready', start );
			}
		} else {
	...
	function initAll() {
		( window.__EVT_FILM_DETAILS || [] ).forEach( initOne );
	}
