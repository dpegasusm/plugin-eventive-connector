( function () {
	...
	function showMessage( el, msg ) {
		...
		el.innerHTML =
			'<div class="eventive-notice" style="text-align:center;">' +
	...
	function getFilter( cfg ) {
		return ( cfg && cfg.__filter ) || 'all';
	}
	function setFilter( cfg, val ) {
		if ( cfg ) {
			cfg.__filter = val || 'all';
		}
	}
	...
	function isModal( el ) {
		return !! (
			el && el.classList.contains( 'eventive-ticket-barcode-modal' )
		);
	}
	...
	function bindEscToClose( modal ) {
		if ( ! modal || modal.__escBound ) {
			return;
		}
		modal.__escHandler = function ( ev ) {
			if ( ev.key === 'Escape' ) {
				closeModal( modal );
			}
		};
	...
	function bindFilterBar( cfg ) {
		...
		const cur = getFilter( cfg );
		try {
			sel.value = cur;
		} catch ( _ ) {}
		sel.addEventListener( 'change', function () {
			setFilter( cfg, sel.value );
			/* re-render below by calling renderer again */ const _tickets =
				( cfg && cfg.__lastTickets ) || [];
			renderTicketsInto( cfg, _tickets, true );
		} );
	}
	...
	function fetchTickets( bucket ) {
		...
		if ( TICKET_CACHE[ key ] ) {
			return Promise.resolve( TICKET_CACHE[ key ] );
		}
		if ( ! window.Eventive ) {
	...
	function isVirtualExpired( t ) {
		...
		if ( ms === null ) {
			return false;
		} // treat null as unknown -> not expired UI
		return ms < Date.now();
	}
	...
	function renderTicketCard( t, idx ) {
		...
		const meta = ticketWhenWhere( t );
		const seat = ticketSeatInfo( t );
		const virt = isVirtual( t );
		let actions =
			'<button class="evt-btn evt-btn-secondary" data-act="barcode">Show Code</button>';
		if ( virt && virtualUrl( t ) ) {
			actions +=
				' <a class="evt-btn evt-btn-primary" data-act="watch" href="' +
				virtualUrl( t ) +
				'" target="_blank" rel="noopener">Watch</a>';
		}

		return (
			'<div class="eventive-ticket-card" data-idx="' +
			idx +
			'">\n' +
			'  <div class="eventive-ticket-card__body">\n' +
			'    <div class="eventive-ticket-card__title">' +
			( title || 'Ticket' ) +
			'</div>\n' +
			'    <div class="eventive-ticket-card__meta">' +
			( meta || '' ) +
			'</div>\n' +
			( seat
				? '    <div class="eventive-ticket-card__seat">' +
				  seat +
				  '</div>\n'
				: '' ) +
			'  </div>\n' +
	...
	function openBarcodeModal( cfg, t ) {
		...
		const body = document.getElementById( cfg.modalBody );
		const close = document.getElementById( cfg.modalClose );
		if ( ! modal || ! body ) {
			return;
		}
		...
		const title = ticketTitle( t );
		const meta = ticketWhenWhere( t );
		const code = ticketQR( t );

		const past = isPastEvent( t );
		const scannedTxt = scannedAtText( t );

		const overlay = past
			? '<div style="position:absolute;inset:0;background:rgba(0,0,0,.55);color:#fff;display:flex;align-items:center;justify-content:center;text-align:center;border-radius:8px;">\
		...
		const scannedBadge = scannedTxt
			? '<div style="text-align:center;margin:6px 0 4px 0;"><span style="display:inline-block;padding:4px 10px;border-radius:999px;background:#dcfce7;color:#14532d;font-weight:700;">Scanned ' +
			  scannedTxt +
			  '</span></div>'
			: '';
		...
		const html =
			...
			'</h3>' +
			( meta
				? '<div style="text-align:center;opacity:.8;margin-bottom:6px;">' +
				  meta +
				  '</div>'
				: '' ) +
			scannedBadge +
			( code
				? '<div style="position:relative;display:flex;justify-content:center;margin-top:6px;">' +
				  '<img src="' +
				  code +
				  '" alt="Ticket QR" style="max-width:320px;width:100%;height:auto;border:1px solid #eee;border-radius:8px;padding:12px;"/>' +
				  overlay +
				  '</div>'
	...
	function renderTicketsInto( cfg, tickets, skipCache ) {
		...
		if ( ! skipCache ) {
			cfg.__lastTickets = tickets;
		}

		if ( ! tickets || ! tickets.length ) {
			list.innerHTML =
				'<h2>Tickets</h2><p>No tickets found for this account.</p>';
			return;
		}
			...
			return arr
				.map( function ( t ) {
					...
					return (
						...
						'<div class="eventive-ticket-card__body">' +
						'<div class="eventive-ticket-card__title">' +
						( ticketTitle( t ) || 'Ticket' ) +
						'</div>' +
						( function () {
							const bits = [];
							const sTxt = scannedAtText( t );
							if ( sTxt ) {
								bits.push(
									'<span class="evt-badge" style="display:inline-block;margin-right:6px;padding:2px 8px;border-radius:999px;background:#dcfce7;color:#14532d;font-weight:700;">Scanned ' +
										sTxt +
										'</span>'
								);
							}
							if ( ! isVirtual( t ) && isPastEvent( t ) ) {
								bits.push(
									'<span class="evt-badge" style="display:inline-block;margin-right:6px;padding:2px 8px;border-radius:999px;background:#e5e7eb;color:#111827;font-weight:700;">Event passed</span>'
								);
							}
							if ( isVirtual( t ) && isVirtualExpired( t ) ) {
								bits.push(
									'<span class="evt-badge" style="display:inline-block;margin-right:6px;padding:2px 8px;border-radius:999px;background:#fee2e2;color:#991b1b;font-weight:700;">Virtual window closed</span>'
								);
							}
							return bits.join( '' );
						} )() +
						...
						( isVirtual( t )
							...
							: '<button class="evt-btn evt-btn-secondary" data-act="barcode">Show Code</button>' +
							  ( getEventId( t )
									? ' <div class="eventive-button" data-vote-event="' +
									  getEventId( t ) +
									  '" data-vote="true"></div>'
									: '' ) ) +
						'</div>' +
		...
		const filterVal = getFilter( cfg );
		let U = upcoming.slice(),
			V = virtuals.slice(),
			P = past.slice();
		...
		list.innerHTML =
			...
			buildFilterBar( cfg ) +
			( U.length
				? '<h3 class="eventive-ticket-section">Upcoming</h3>' +
				  renderWithRefs( U )
				: '' ) +
			( V.length
				? '<h3 class="eventive-ticket-section">Virtual</h3>' +
				  renderWithRefs( V )
				: '' ) +
			( P.length
				? '<h3 class="eventive-ticket-section">Past</h3>' +
				  renderWithRefs( P )
				: '' );
		...
		list.querySelectorAll(
			'.eventive-ticket-card [data-act="barcode"]'
			...
			btn.addEventListener( 'click', function () {
				const card = btn.closest( '.eventive-ticket-card' );
				const idx = card
					? parseInt( card.getAttribute( 'data-idx' ), 10 )
					: -1;
				const t = cards[ idx ];
				if ( ! t ) {
					return;
	...
	function initOne( cfg ) {
		...
		const login = document.getElementById( cfg.login );
		const list = document.getElementById( cfg.list );
		if ( ! wrap || wrap.__inited ) {
			return;
		}
		wrap.__inited = true;
		...
		const fallback = setTimeout( function () {
			if ( ! window.Eventive || ! Eventive.on ) {
				if ( ! inParentAccountContainer( wrap ) ) {
					showMessage(
						list,
						'Eventive is not available. Please check your connection and try again.'
					);
					hide( login );
					show( list );
				}
			...
			Eventive.on( 'ready', function () {
				...
				try {
					if ( Eventive.isLoggedIn() ) {
						hide( login );
						show( list );
						fetchTickets( cfg.bucket )
							.then( function ( tickets ) {
								renderTicketsInto( cfg, tickets );
							} )
							.catch( function ( err ) {
								console.error(
					...
					} else if ( ! inParentAccountContainer( wrap ) ) {
						showMessage(
							list,
							'You are not logged in. Please log in to view your tickets.'
						);
						hide( login );
						show( list );
					} else {
					...
					showMessage(
						list,
						'There was a problem loading your tickets.'
					);
					hide( login );
					show( list );
				}
	...
	function initAll() {
		const cfgs = window.__EVT_ACCOUNT_TICKETS || [];
		if ( ! cfgs.length ) {
			return;
		}
