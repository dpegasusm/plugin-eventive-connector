( function () {
	...
	function initContainer( wrap ) {
		...
		const content = wrap.querySelector(
			'[id$="_content"], #eventive-account-content, .eventive-account-content'
		);
		const loginReq = wrap.querySelector(
			'[id$="_login_required"], #eventive-login-required-message, .eventive-login-required'
		);
		const actions = wrap.querySelector( '.account-actions' );
		const logoutLnk = wrap.querySelector(
			'[id$="_logout_link"], #eventive-logout-link, .eventive-logout-link'
		);
		...
		function bindLogout() {
			if ( ! logoutLnk || logoutLnk.__evtBound ) {
				return;
			}
			...
			logoutLnk.addEventListener( 'click', function ( e ) {
				...
				try {
					...
					document.cookie =
						'eventive-personState=; path=/; max-age=0';
		...
		function run() {
			try {
				if (
					Eventive &&
					typeof Eventive.isLoggedIn === 'function' &&
					Eventive.isLoggedIn()
				) {
					showContent();
				} else {
	...
	function initAll( root ) {
		( root || document )
			.querySelectorAll( '.eventive-account-container' )
