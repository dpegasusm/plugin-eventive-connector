...
if ( ! is_admin() ) {
	add_action(
		...
		function () {
			...
			wp_enqueue_style( 'eventive-mode-style', $style_base_url . $style_to_enqueue, array(), $mode_ver );
			...
			$sc_to_css = array(
				'eventive-login'             => array( 'eventive-login.css' ),
				'eventive-account'           => array(
					'eventive-account.css',
					'eventive-account-details.css',
					'eventive-account-passes.css',
					'eventive-account-tickets.css',
				),
				'eventive-account-details'   => array( 'eventive-account-details.css' ),
				'eventive-account-passes'    => array( 'eventive-account-passes.css' ),
				'eventive-account-tickets'   => array( 'eventive-account-tickets.css' ),
				'eventive-film-guide'        => array( 'eventive-film-guide.css' ),
				'eventive-calendar'          => array( 'eventive-calendar.css' ),
				'eventive-marquee'           => array( 'eventive-marquee-carousel.css' ),
				'eventive-native-year-round' => array( 'eventive-calendar.css' ),
				'eventive-events'            => array( 'events-events.css' ),
				'eventive-events-list'       => array( 'events-list.css' ),
				'eventive-tags'              => array( 'eventive-tags.css' ),
			);
			...
			if ( ! empty( $present ) ) {
				...
				$enqueue = function ( $handle, $file ) use ( $style_base_dir, $style_base_url ) {
					...
					if ( ! file_exists( $path ) ) {
						return; }
					$ver = filemtime( $path );
					wp_enqueue_style( $handle, $style_base_url . $file, array( 'eventive-mode-style' ), $ver );
				};
				...
				foreach ( $present as $key => $true ) {
					if ( isset( $sc_to_css[ $key ] ) ) {
						$add_css( $sc_to_css[ $key ] ); }
				}
				if ( isset( $block_to_css ) ) {
					foreach ( $present as $key => $true ) {
						if ( isset( $block_to_css[ $key ] ) ) {
							$add_css( $block_to_css[ $key ] ); }
					}
				}
...
add_filter(
	...
	function ( $classes ) {
		...
		$classes   = array_diff( $classes, array( 'eventive-dark-mode', 'eventive-light-mode', 'eventive-default-mode' ) );
