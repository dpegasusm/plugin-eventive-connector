...
function fetch_eventive_donations( WP_REST_Request $request ) {
	...
	if ( empty( $start_time ) || empty( $end_time ) ) {
		return new WP_Error( 'missing_params', 'Start and end times are required.', array( 'status' => 400 ) );
	}
	...
	if ( ! $api_key ) {
		return new WP_Error( 'missing_api_key', 'API key is not configured.', array( 'status' => 500 ) );
	}

	$response = wp_remote_get(
		'https://api.eventive.org/ledger/transactions?expand=person&date=' . urlencode(
			json_encode(
				array(
					'$gt' => $start_time_iso,
					'$lt' => $end_time_iso,
	...
	if ( is_wp_error( $response ) ) {
		return new WP_Error( 'api_error', 'Failed to fetch donations.', array( 'status' => 500 ) );
	}
...
function eventive_update_person_details( WP_REST_Request $request ) {
	...
	if ( ! $person_id || empty( $params ) ) {
		return new WP_Error( 'missing_params', 'Missing person_id or update payload.', array( 'status' => 400 ) );
	}
	...
	$response_existing = wp_remote_get(
		"https://api.eventive.org/people/{$person_id}",
		array(
			'headers' => array(
				'x-api-key' => $api_key,
			),
	...
	if ( is_wp_error( $response_existing ) ) {
		...
		return new WP_Error( 'api_error', $response_existing->get_error_message(), array( 'status' => 500 ) );
	}

	$existing_body    = wp_remote_retrieve_body( $response_existing );
	$existing_data    = json_decode( $existing_body, true );
	$existing_details = $existing_data ?? array();
...
function eventive_update_pass_details( WP_REST_Request $request ) {
	...
	if ( ! $pass_id || empty( $params ) ) {
		return new WP_Error( 'missing_params', 'Missing pass_id or update payload.', array( 'status' => 400 ) );
	}
	...
	$response_existing = wp_remote_get(
		"https://api.eventive.org/passes/{$pass_id}",
		array(
			'headers' => array(
				'x-api-key' => $api_key,
			),
